    ...
    <div class="p-4 sm:ml-64 bg-gray-200 min-h-screen">
        <div class="p-4 mt-14">
            ...
            <div class="bg-white rounded-lg shadow mb-6">
                ...
                <!-- Filter Content -->
                <form action="{{ route('admin.settlement') }}" method="GET">
                    <div id="filter-content" class="px-4 sm:px-6 pb-6 hidden">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                            ...
                            <div class="space-y-2">
                                <label for="search"
                                    class="block text-[9px] sm:text-[9px] md:text-[10px] lg:text-[11px] font-medium text-gray-800">Search</label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 20 20" fill="currentColor">
                                    ...
                                    <input type="text" id="search" name="search"
                                        ...
                                        class="block  w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-[9px] sm:text-[9px] md:text-[10px] lg:text-xs">
                                </div>
                            </div>
                            ...
                            <div class="flex items-end gap-2">
                                <button id="search-btn"
                                    class="bg-blue-600 hover:bg-blue-700 text-white text-[9px] sm:text-[9px] md:text-[10px] lg:text-[11px] px-4 py-2 rounded-md transition-colors w-full flex items-center justify-center">
                        ...
                        <tbody id="transactions-body" class="bg-white divide-y divide-gray-200">
                            @foreach ($tnx as $item)
                                <tr class="transaction-row hover:bg-gray-50">
                                    ...
                                    <td
                                        class="px-3 py-3 whitespace-nowrap text-[9px] sm:text-[9px] md:text-[10px] lg:text-[11px] text-center font-medium text-gray-900">
                                        {{ ($tnx->currentPage() - 1) * $tnx->perPage() + $loop->iteration }}
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap text-center">
                                        <div class="">
                                            <span
                                                class=" text-[9px] sm:text-[9px] md:text-[10px] lg:text-[11px] ">{{ !empty($item['transactionId']) ? $item['transactionId'] : 'N/A' }}</span>
                                        </div>
                                    </td>
                                    <td
                                        class="px-3 py-3 whitespace-nowrap text-[9px] sm:text-[9px] md:text-[10px] lg:text-[11px] text-center text-gray-500 ">
                                        {{ $item['merchantInvoiceNo'] }}
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap">
                                        <div
                                            class="text-[9px] sm:text-[9px] md:text-[10px] lg:text-[11px] text-center text-gray-900 truncate max-w-[120px]">
                                            {{ $item['amount'] }}
                                        </div>
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap">
                                        <div
                                            class="text-[9px] sm:text-[9px] md:text-[10px] lg:text-[11px] text-center text-gray-900">
                                            {{ $item['currency'] }}</div>
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap text-center">
                                        @if ($item['status'] == 'SUCCESS')
                                            <span
                                                class="px-3 inline-flex text-[9px] leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                                {{ $item['status'] }}
                                            </span>
                                        @elseif ($item['status'] == 'FAIL')
                                            <span
                                                class="px-3 inline-flex text-[9px] leading-5 font-semibold rounded-full bg-red-100 text-red-800 ml-2">
                                                {{ $item['status'] }}
                                            </span>
                                        @elseif ($item['status'] == 'PENDING')
                                            <span
                                                class="px-3 inline-flex text-[9px] leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                                                {{ $item['status'] }}
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap text-center">
                                        <div class="flex items-center">
                                            <span
                                                class="ml-2 text-[9px] sm:text-[9px] md:text-[10px] lg:text-[11px] ">{{ $item['merchantFeeRate'] }}</span>
                                        </div>
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap text-center">
                                        <div class="">
